...
//echo "style=".$this->style;
	   ...
	   if(!file_exists($this->path."includes/styles/".$this->style."/style.css"))
             $this->style="default";
           echo  "<link href='".$this->htmlpath."includes/styles/".$this->style."/style.css' rel='stylesheet' type='text/css' />\n";
           if(file_exists($this->path."includes/styles/".$this->style."/common.css"))
             echo  "<link href='".$this->htmlpath."includes/styles/".$this->style."/common.css' rel='stylesheet' type='text/css' />\n";
           if(file_exists($this->path."includes/js/script.js"))
             echo  "<script type='text/javascript' src='".$this->htmlpath."includes/js/script.js'></script>";
//echo "style=".$this->style;
    }
...
//echo "style:".$this->style.",".$symbol;
        	echo "\n<li class='".$liclass."' id='".$node->GetId()."'>";
        	// displaying plus or minus sign to the left of the text
        	if($last)
        	   echo "<img src='".$this->htmlpath."includes/styles/".$this->style."/images/".$symbol."-last.jpg' class='image-last' onclick='Switch(this,\"".$this->style."\",\"".$this->htmlpath."\")'></img>";
        	else
        	   echo "<img src='".$this->htmlpath."includes/styles/".$this->style."/images/".$symbol.".jpg' class='image' onclick='Switch(this,\"".$this->style."\",\"".$this->htmlpath."\")'></img>";
   ...
   echo CHtml::link($node->GetCaption(), array('site/page', 'view'=>'iframe', 'page'=>'http://localhost/own/joorgportal/'.$node->GetFile() ))."</span>"; 

//        echo $node->GetCaption().":".$node->GetFile()."</span>";
...
class Node
{
	...
	*/
    public function AddNode($caption,$file="")
    {
        ...
        $id=$this->GetId()."_".++$this->numChildren;
//   echo CHtml::link($caption, array('site/page', 'view'=>'iframe', 'page'=>'http://localhost/own/joorgportal/'.$file)); 
...
//    echo CHtml::link($this->GetId(), array('site/page', 'view'=>'iframe', 'page'='http://localhost/own/joorgportal/'.$this->file));
//$this->SetBaseUrl('http://localhost/own/joorgportal/'.$this->file);   
//echo Yii::app()->request->scriptUrl."<br/>" ;
//    echo $this->file;
//$url = Yii::app()->request->createUrl($this->file);

    $winwidth=1035;
    $winheight=580;
//    echo $this->file;
//    echo "            <iframe src='http://localhost/own/joorgportal/".$this->file."' name='Fensterlein' width='".$winwidth."' height='".$winheight."'";
