...
class DbDiff {
	 ...
	 * @return void
	 */
	function compare($schema1, $schema2, $comp_table) {
		
		$tables1 = array_keys($schema1['tables']);
		$tables2 = array_keys($schema2['tables']);
		
		if ($comp_table<>"") {
		  $tables = array($comp_table);
		} else {
		...
		foreach ($tables as $table_name) {
			...
			foreach ($fields as $field_name => $field) {
				...
				if (!isset($schema1['tables'][$table_name][$field_name])) {
					...
					$results[$table_name][] = '<em>' . $schema1['dbname']
				...
				if (!isset($schema2['tables'][$table_name][$field_name])) {
					...
					$results[$table_name][] = '<em>' . $schema2['dbname']
						. '</em> is missing field: <code>' . $field_name
						. '</code>';
	...
	function createscript($schema1, $schema2, $comp_table) {
		$tables1 = array_keys($schema1['tables']);
		$tables2 = array_keys($schema2['tables']);
		...
		foreach ($tables as $table_name) {
			if (!isset($schema1['tables'][$table_name])) {
				...
				$results[$table_name][] = 'CREATE TABLE '.$table_name;
				
				continue;
			}
			if (!isset($schema2['tables'][$table_name])) {
				
				$results[$table_name][] = 'DROP TABLE '.$table_name;
			...
			foreach ($fields as $field_name => $field) {
				...
				if (!isset($schema2['tables'][$table_name][$field_name])) {
					...
					$query = 'ALTER TABLE ' . $table_name . ' ADD COLUMN ' . $field_name .' ' .$schema1['tables'][$table_name][$field_name]['Type'];
               if ($schema1['tables'][$table_name][$field_name]['Null']==="NO") {
                 $query = $query ." NOT NULL";
               }	    				
               if (isset($schema1['tables'][$table_name][$field_name]['Default'])) {
                 if ($schema1['tables'][$table_name][$field_name]['Default']==="CURRENT_TIMESTAMP") {
                 //  $query = $query ." default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP ";
                   $query = $query ." default ".$schema1['tables'][$table_name][$field_name]['Default'];
                 } else {
                   $query = $query ." default '".$schema1['tables'][$table_name][$field_name]['Default']."'";
                 }
               }	    				
               if ($schema1['tables'][$table_name][$field_name]['Extra']<>"") {
                 $query = $query ." ". $schema1['tables'][$table_name][$field_name]['Extra'];
               }	    				
					$query = $query . ";".chr(13).chr(13);
               $results[$table_name][] = $query;
