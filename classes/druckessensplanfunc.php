...
function druckessensplan($datum,$dbtable,$prtfield,$prtuser,$orderby,$hoehe,$anzzl) {
include ("../config.php");
if ($orderby<>"") {
    ...
    while ($line = mysql_fetch_array($result)) {
      ...
      if ($wert == '') {
        ...
        if ($id!="") {
          ...
          $wert=substr($line['fldzeit'],0,5)." ".$fline['fldBez']." (170 kcal)";       
        } else {
          $wert=".";
        }  
      } else {
        $kcal=$line['fldkalorien'];	
        if ($kcal == '') {
          $id=$line[fldid_rezepte];
          if ($id!="") {
            $fquery = "SELECT * FROM tblrezepte WHERE fldIndex=".$id;
            //echo $fquery."<br>";
            $fresult = mysql_query($fquery) or die(mysql_error().$fquery);            $fline = mysql_fetch_array($fresult);
            $kcal=$fline['fldkalorien'];
            $wert=substr($line['fldzeit'],0,5)." ".$line[$prtfield]." (".$kcal." kcal)";
          }         
        }	
        $wert=substr($line['fldzeit'],0,5)." ".$line[$prtfield]." (".$kcal." kcal)";
      } 
...
function druckessensplanauswahl($dbtable,$prtfield,$prtuser,$orderby) {
