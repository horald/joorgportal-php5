...
echo "<table>";
  for ($ielem = 0; $ielem <= $anzelem; $ielem++)  
  //echo "<tr>";
  for ($i = 0; $i <= $anzsp; $i++) {
  {
    //echo "<td width='500px'>";
...
function updatesavedirect($pararray,$listarray,$filterarray,$filter,$idwert,$menu,$dbname,$errmsg) {
  ...
  if (isset($_REQUEST['submit'])) { 
    ...
    foreach ( $listarray as $arrelement )
    {
      if ($arrelement['fieldsave']<>'NO') {
        if (($arrelement['type']=='hostname') OR ($arrelement['type']=='ipaddr') OR ($arrelement['type']=='blob') OR ($arrelement['type']=='zahlid') OR ($arrelement['type']=='pos') OR ($arrelement['type']=='textarea') OR ($arrelement['type']=='blutdruck') OR ($arrelement['type']=='average') OR ($arrelement['type']=='YN') OR ($arrelement['type']=='text') OR ($arrelement['type']=='procent') OR ($arrelement['type']=='blobid') OR ($arrelement['type']=='image') OR ($arrelement['type']=='calctext') OR ($arrelement['type']=='select') OR ($arrelement['type']=='selectid') OR ($arrelement['type']=='zahl') OR ($arrelement['type']=='date') OR ($arrelement['type']=='calc')) {
    ...
    if ($pararray['DBSYNC']='OK') {
      $strset=$strset.",flddbsyncstatus='SYNC'";
    }  
    ...
    if ($pararray['dbsyncnr']=="J") {
      ...
      $timestamp=$datetime->format('Y-m-d H:i:s');  
  	   $strset=$strset.",fldtimestamp='".$timestamp."'";
    }
    ...
    if ($resync==true) {
      ...
      $qrysync = "UPDATE ".$pararray['dbtable']." SET fldtimestamp='2016-02-01 08:00:00', flddbsyncnr=1 WHERE ".$pararray['fldindex']."='$idwert'";
      mysql_query($qrysync) or die("Error using mysql_query($qrysync): ".mysql_error());
    }
    $dscopy=$_POST['dscopy'];
    if ($dscopy==true) {
...
function updatesave($pararray,$listarray,$filterarray,$filter,$idwert,$menu,$dbname,$menuid,$errmsg) {

  echo "<a class='btn btn-primary' href='showtab.php?menu=".$menu."&idwert=".$menuid."'>zurueck</a><br>";
  if (isset($_REQUEST['submit'])) { 
    updatesavedirect($pararray,$listarray,$filterarray,$filter,$idwert,$menu,$dbname,$errmsg);
  } else {
    echo "<div class='alert alert-warning'>";
    echo "Der Vorgang wurde abgebrochen.<br>"; 
    echo "</div>";
