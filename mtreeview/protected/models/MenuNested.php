 ...
 */
class MenuNested extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return MenuNested the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('title, lft, rgt, url, visible, htmlOptions', 'required'),
			array('visible', 'numerical', 'integerOnly'=>true),
			array('title, url', 'length', 'max'=>255),
			array('lft, rgt', 'length', 'max'=>10),
			array('task', 'length', 'max'=>64),
			array('htmlOptions', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, title, lft, rgt, url, visible, task, htmlOptions', 'safe', 'on'=>'search'),
		);
	}
